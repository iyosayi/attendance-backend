...
const bulkCreateCampers = async () => {
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!email || !isValidEmail(email)) {
        ...
        autogeneratedEmails.push(
          `Row ${rowNumber}: ${firstName} ${lastName} - Original: ${originalEmail} → Generated: "${email}"`
        );
      }
    ...
    if (autogeneratedEmails.length > 0) {
      console.log('⚠️  Autogenerated emails (missing or invalid email format):');
    ...
    if (campersToInsert.length === 0) {
      console.log('⚠️  No valid campers to import');
      await mongoose.disconnect();
      process.exit(0);
    }
    ...
    if (campersToCreate.length === 0) {
      console.log('⚠️  All campers already exist in the database');
      if (duplicates.length > 0) {
        console.log('\n⚠️  Existing campers:');
    ...
    console.log(`🚀 Creating ${campersToCreate.length} campers...\n`);
    ...
    if (duplicateEmails.length > 0) {
      console.log('⚠️  Duplicate emails (skipped during insert):');
